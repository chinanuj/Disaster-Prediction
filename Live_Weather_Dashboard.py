...
API_URL = "https://api.open-meteo.com/v1/forecast?latitude=40.7128&longitude=-74.0060&hourly=temperature_2m,relative_humidity_2m,windspeed_10m&timezone=auto"
...
app.layout = html.Div([
    ...
    html.Div(
        dcc.Slider(
            ...
            step=1,
            value=12,
            marks={i: f'{i}h' for i in range(1, 49, 6)},
...
def fetch_weather():
    ...
    timestamps = response["hourly"]["time"][-48:]  # Last 48 hours
    data = {
        "Temperature (°C)": response["hourly"]["temperature_2m"][-48:],
        "Humidity (%)": response["hourly"]["relative_humidity_2m"][-48:],
        "Wind Speed (m/s)": response["hourly"]["windspeed_10m"][-48:]
    }
